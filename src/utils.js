...
const compareFaces = ["M", "L", "K", "J", "I", "H", "G", "F", "E", "D", "C", "B", "A"];
...
export const createDeck = () => {
  ...
  for(let i = 0; i < suits.length; i++) {
		for(let j = 0; j < faces.length; j++) {
			var card = {face: faces[j], compareFace: compareFaces[j], suit: suits[i]};
			cards.push(card);
		}
	}
  return cards;
...
export const getRank = (cards) => {
  ...
  const compareCountAndFacec = (face1, face2) => {
    ...
    if (face2 > face1) return -1; // face1 should before face2
  ...
  const straight = facesInOrder.every((f, index) => f.charCodeAt(0) - first === index)
  const rank = (flush && straight && facesInOrder[0] === 'A' && 1) ||
  (flush && straight && 2) ||
  (duplicates[4] && 3) ||
